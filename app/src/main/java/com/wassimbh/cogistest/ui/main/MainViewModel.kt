...
class MainViewModel  @Inject constructor(
    ...
    private fun handleJSONFiles(fileName: String, strValue: String){
    ...
    private fun initItineraryPoints(jsonStr: String) {
        ...
        for (i in 0 until edgesArray.length()) {
            ...
            val beginPoint = fromLngLat(itineraryPoints[begin].longitude, itineraryPoints[begin].latitude)
            ...
            val floorValue = if(itineraryPoints[begin].floor == itineraryPoints[end].floor) itineraryPoints[begin].floor else null
    ...
    fun getClosestEdge(beginPoint: Point, destination: Point): GraphEdge?{
        var finalEdge: GraphEdge? = null
        edgesLiveData.value?.let { edgesList->
            finalEdge = getEdgeFromList(beginPoint, edgesList, true)
            finalEdge?.let {e->
                val relatedEdges = edgesList.filter { it.start == e.start }
                finalEdge = getEdgeFromList(destination, relatedEdges, false)
            }
        }

        return finalEdge
    }
    fun getClosestEdges(beginPoint: Point, destination: Point, destinationFloor: Int): List<GraphEdge>?{
        var beginningEdges: List<GraphEdge>? = null
        var destinationEdges: List<GraphEdge>? = null
        edgesLiveData.value?.let { edgesList->
            getEdgeFromList(beginPoint, edgesList, true)?.let { e->
                beginningEdges = edgesList.filter { it.start == e.start && it.floorValue == 0 }
            }
            getEdgeFromList(destination, edgesList, false)?.let { e->
                destinationEdges = edgesList.filter { it.destination == e.destination }
            }
        }
    ...
    fun getClosestChangingFloorEdges(startPoint: PoiEntity, endPoint: PoiEntity, edges: List<GraphEdge>): GraphEdge?{
        ...
        edges.filter {it.floorChange}
            .forEach{
                ...
                val toTargetFloor = (it.from == endPoint.getFloorValue() || it.to == endPoint.getFloorValue())
    ...
    fun getEdgeFromList(point:Point, edges: List<GraphEdge>, calculateBeginning: Boolean): GraphEdge?{
        var edge: GraphEdge? = null
